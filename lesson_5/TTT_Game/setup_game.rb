...
class SetupGame
  ...
  def game_starting_player
    ...
    @starting_player = coin_toss.winner
    coin_toss.display_winner
    prompt_to_continue("Press enter to proceed in the setup of the game.")
  end
  ...
  def select_option(option)
    case option
    when 1 then players_names
    when 2 then game_variation
    when 3 then max_score
    when 4 then players_markers
    when 5 then game_starting_player
    end
  end
